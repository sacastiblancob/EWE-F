...
!                    ***************************
  (n,step, imonth, time, tstep, es_data, BB, BB_spatial, flow2detritus, &
...
! INTENT VARIABLES
  ...
  REAL(RLEN), INTENT(INOUT) :: BB(nvars)
  ...
  REAL(RLEN), INTENT(INOUT) :: mat_out(nlat, nlon, noftsteps, nvars)
  REAL(RLEN), INTENT(OUT) :: rel_out(nlat, nlon, noftsteps, nvars)
  ...
  do lon = 1, nlon
    do lat = 1, nlat
      if (grid(lat, lon) == 1) then
        ...
        call calculateFishingMortalities(nvars, imonth, BB, &
                                         force, ep_data, es_data)

!       do n = 1, StepsPerMonth

        ! call the Runge-Kutta 4th order numeric ode solver
        call rk4(nvars, BB, time, tstep, integrate, lat, lon, ep_data, &
        ...
        call ecospace(nvars, nlat, nlon, lat, lon, time, BB, &
