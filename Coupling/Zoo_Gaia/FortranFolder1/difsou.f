...
!-----------------------------------------------------------------------
! ADDED BY SERGIO
! KILLING ALL INITIAL CONDITIONS WHERE H<=0
      IF((AT-AT0).LE.(DT+0.001))THEN
        DO III=1,NPOIN
          IF(HPROP%R(III).LE.0.01)THEN
            DO JJJ=1,NTRAC
              TN%ADR(JJJ)%P%R(III) = 0D0
            ENDDO
          ENDIF
        ENDDO
      ENDIF
...
!
! ENSURING ZERO OF TRACERS ON DRY ZONES
!
      DO III=1,NPOIN
      !ENSURGIN ZERO OF TRACERS VOIDS ON DRY ZONES  
        IF(HPROP%R(III).LE.0.01)THEN
          DO JJJ=1,NTRAC
            TEXP%ADR(JJJ)%P%R(III) = 0D0
            TIMP%ADR(JJJ)%P%R(III) = 0D0
          ENDDO
        ENDIF
      !CHANGING DIFFUSITIVITY FOR TRACER LOCATED IN NEAR DRY AREAS
        IF(HPROP%R(III).LE.0.1)THEN
          DO JJJ=1,NTRAC
            VISCT%ADR(JJJ)%P%R(III) = 0.0
          ENDDO
        ELSE
          DO JJJ=1,NTRAC
            VISCT%ADR(JJJ)%P%R(III) = DIFNU
          ENDDO
        ENDIF
!        
      ENDDO
!      WRITE(*,*)'NTRAC',NTRAC,IND_SED
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
