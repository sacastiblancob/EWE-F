...
real(RLEN), allocatable, target :: ep_diet(:, :)
real(RLEN), allocatable, target :: ep_detfate(:, :)
real(RLEN), allocatable :: det_export_rate(:)
real(RLEN), allocatable :: flow2detritus(:)

character (len = 250) :: HDF5_fname

!#ifdef isWithBFM
!  integer, allocatable :: idxBBc(:),idxBBn(:),idxBBp(:), idxBBl(:), idxBBi(:), idxBBs(:)
!#endif
