...
!========================================================================
                  ...
                  subroutine initialiseSplitGroups &
  (nvars, nstanzas, B, ep_data, ms_data, es_data, es_ms_data)
...
!write(*,*) ms_data(1)%SplitNo
!write(*,*) ms_data(1)%Wage

do stanza = 1, nstanzas

!    write(*,*) "COMP"
!    write(*,*) stanza
!    write(*,*) ms_data(stanza)%age_infinity
!    write(*,*)
    Be = 0
    do age = 0, ms_data(stanza)%age_infinity

!        write(*,*) age,ms_data(stanza)%age_infinity
!        write(*,*)
        es_ms_data(stanza)%NageS(age) = ms_data(stanza)%SplitNo(age)
        es_ms_data(stanza)%WageS(age) = ms_data(stanza)%Wage(age)
!        write(*,*) es_ms_data(stanza)%NageS(age)
!        write(*,*) es_ms_data(stanza)%WageS(age)
!        write(*,*) ms_data(stanza)%Wmat_Winf
        ...
        if (es_ms_data(stanza)%WageS(age) &
          > ms_data(stanza)%Wmat_Winf) then
            ...
            Be = Be + es_ms_data(stanza)%NageS(age) &
              * es_ms_data(stanza)%EggsSplit(age)
!            write(*,*) es_ms_data(stanza)%EggsSplit(age)
!            write(*,*) Be
!            write(*,*)
        end if
    end do
...
!    write(*,*) es_ms_data(stanza)%BaseEggsStanza
!    write(*,*) es_ms_data(stanza)%EggsStanza
!    write(*,*) es_ms_data(stanza)%RscaleSplit
!    write(*,*)
end do

! calculate the number of biomasses in split groups
 call setSplitPred(nvars, nstanzas, B, ms_data, es_data, es_ms_data)
