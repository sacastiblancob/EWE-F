...
use statevartypesecopath, only: HDF5_fname, ep_data, ep_growth, &
                          ep_diet, ep_detfate, nstanzas
use hdf5
...
character(len = 9), parameter   :: ms_setname = "ep_growth"
...
call h5tarray_create_f(H5T_NATIVE_REAL, 2, adims, memtype, hdferr)

! read the data
f_ptr_dc = c_loc(ep_diet)
 call h5dread_f(dset_id, memtype, f_ptr_dc, hdferr)
...
f_ptr_df = c_loc(ep_detfate)
 call h5dread_f(dset_id, memtype, f_ptr_df, hdferr)
