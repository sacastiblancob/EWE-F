...
do j = 1, ndetritus
    ...
    do var = 1, nvars
        ...
        if (ep_data(var)%org_type /= 0) then 
            ...
            flow2detritus(j) = flow2detritus(j) + ((1 - ep_data(var)%EE) &
              * ep_data(var)%PoB) * biomass(var) * ep_detfate(var, j)
             ! WRITE(*,*) 'EE ', var, ep_data(var)%EE
             ! WRITE(*,*) 'PoB ', var, ep_data(var)%PoB
             ! WRITE(*,*) 'biomass ', var, biomass(var)
             ! WRITE(*,*) 'detfate ', var, j, ep_detfate(var, j)
        end if
    ...
    !WRITE(*,*) 'flow2detritus ', flow2detritus(j)
end do
...
do j = 1, ndetritus
    ...
    !WRITE(*,*) 'flow2detritus 2 ', flow2detritus(j)
end do
#endif
...
if (FirstTime .eqv. .true.) then
    do j = 1, ndetritus
        det_export_rate(j) = (flow2detritus(j) &
          ...
          !WRITE(*,*) 'det_export_rate ', det_export_rate(j)
    end do
end if
