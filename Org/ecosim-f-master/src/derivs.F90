...
! P/B scaled by Max. Relative Production, i.e. es_data%pob_max.
        if (time == 0. .and. imonth == 0) then
            ...
            if (ep_data(var)%org_type /= 0) then
                ...
                !WRITE(*,*) 'PoB_biomass ', es_data(var)%PoB_biomass
            end if            
        else
#ifdef _ForcePrimaryProd_
    ...
    Pmult = 1.0
        ...
        !WRITE(*,*) 'ES_POB_BASE ', es_data(var)%PoB_base
        !WRITE(*,*) 'rel_PoB_max ', es_data(var)%rel_PoB_max
        !WRITE(*,*) 'PoB ', ep_data(var)%PoB
        !WRITE(*,*) 'PoB_biomass ', es_data(var)%PoB_biomass
    end if
end do
...
do pred = 1, nvars
    ...
    !WRITE(*,*) 'es_qq ', es_data(pred)%qq
    !WRITE(*,*) 'es_EatenBy ', es_data(pred)%EatenBy
end do
...
do var = 1, nvars
    ...
    !WRITE(*,*) 'es_unass ', es_data(var)%unassimilated
end do
...
#endif
    !WRITE(*,*) 'es_M2 ', es_data(prey)%M2
    !WRITE(*,*) 'es_EatenOf ', es_data(prey)%EatenOf
end do
...
do var = 1, nvars
    ...
    !WRITE(*,*) 'Var, Xdot ', var, xdot(var)
    !WRITE(*,*) 'Var, biomeq ', var, biomeq(var)
end do
